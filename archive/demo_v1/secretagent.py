...
def program_trace_prompt_llm(func, *args, **kw):
    ...
    """
    # Simplified prompt template (inline since we don't have prompts/ folder)
    template_text = """You are executing a Python function. Given the function definition and inputs, predict the output.
...
Instructions:
1. Read the function docstring carefully for examples
2. Follow the pattern shown in the examples
3. Wrap your final answer in <answer>...</answer> tags
4. The answer should be just the return value, nothing else

Now execute the function and provide the answer:"""
