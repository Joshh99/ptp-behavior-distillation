...
from benchmark.rq1.dataset.airline_loader import AirlineLoader
from benchmark.rq1.experiments.l1_ptool_extraction import (
    baggage_allowance_l1_ptool,
    baggage_allowance_l1_transparent,
)
from benchmark.rq1.experiments.cot_baseline import run_cot_baseline
from benchmark.rq1.experiments.tool_aug_baseline import run_tool_augmented
...
def estimate_cost(
    ...
    total_calls = num_problems * len(complexity_levels) * len(experiments)
    estimates = {}
    total_cost = 0.0

    for exp_name in experiments:
        tokens = AVG_TOKENS_PER_CALL.get(exp_name, {"input": 2000, "output": 500})
...
def save_checkpoint(output_path: Path, all_results: List[Dict], metadata: Dict[str, Any]) -> None:
...
def run_full_baseline(
    ...
    model: str = "meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo",
    output_file: str = "results/rq1/baseline_results.json",
    resume: bool = True,
    ...
    if experiment_filter:
        experiments = {k: v for k, v in all_experiments.items() if k in experiment_filter}
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run RQ1 baseline experiments (airline domain)")
    parser.add_argument(
        "--num-problems",
        type=int,
        default=5,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/rq1/baseline_results.json",
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        choices=["cot", "tool_aug", "l1_pure", "l1_transparent"],
        default=None,
        help="Experiment types to run (default: all four)"
    )
    parser.add_argument(
        "--yes", "-y",
        action="store_true",
    ...
    experiments_to_run = experiment_filter or ["l1_pure", "l1_transparent", "cot", "tool_aug"]

    estimate = estimate_cost(args.num_problems, args.complexity, args.model, experiments_to_run)

    print("=" * 80)
    print("RQ1 BASELINE EXPERIMENTS")
    print("=" * 80)
