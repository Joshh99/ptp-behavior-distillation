...
class ModelSelector:
    ...
    def _filter_by_criteria(
        ...
        if not filtered:
            logger.warning(
                f"No candidates passed filter criteria (cost<={criteria.max_cost_per_call}, "
                f"latency<={criteria.max_latency_ms}ms, quality>={criteria.min_quality_threshold}). "
                f"Returning all {len(candidates)} candidates as fallback."
            )
            return candidates
        return filtered
