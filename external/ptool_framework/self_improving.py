...
from .react import ReActAgent, ReActResult, ReActTrajectory, ReActStep, Action, Observation
from .traces import StepStatus
...
TASK: {goal}"""
    ...
    def _learn_from_repair(
        ...
        # repair_result.repaired_trace is a WorkflowTrace, not a ReActTrajectory
        # Skip pattern extraction if we can't provide a distinct repaired trajectory
        if not repair_result.repaired_trace:
            return

        # Build a minimal repaired trajectory from the repaired trace
        repaired_trajectory = ReActTrajectory(
            goal=goal,
            trajectory_id=f"repair_{result.trajectory.trajectory_id}",
        )
        for i, step in enumerate(repair_result.repaired_trace.steps):
            repaired_trajectory.steps.append(ReActStep(
                action=Action(
